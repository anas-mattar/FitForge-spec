...
```text
Backend  (run in fitforge-api/): dotnet build --warnaserror && dotnet test
Frontend (run in fitforge-web/): npm run lint && npm run typecheck && npm run build && npm test
```

Run each gate from its own repository root — a gate run from the governance root proves
nothing. `npm run typecheck` is listed explicitly rather than relying on `npm run build`:
...
```bash
dotnet build --warnaserror && dotnet test; echo "EXIT: $?"                        # in fitforge-api/
npm run lint && npm run typecheck && npm run build && npm test; echo "EXIT: $?"   # in fitforge-web/
```

PowerShell:

```powershell
dotnet build --warnaserror && dotnet test   # in fitforge-api/
"EXIT: $LASTEXITCODE"
# frontend, in fitforge-web/: npm run lint && npm run typecheck && npm run build && npm test
```
