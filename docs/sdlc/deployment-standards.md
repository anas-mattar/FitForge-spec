...
1. Conditions (all-of): FitForge is deployed and serving members, AND the fault loses or
   corrupts member training data or blocks sign-in for all members, AND no gate-passing fix
   exists within 4 hours. A styling defect, a slow page, or a single member's bad row is
   never a break-glass condition.
2. Named approver: Anas (project owner). Not "whoever is online" — if Anas is unreachable,
   the change waits or is reverted, because a two-person team has no quorum to invent.
3. Revert window: the bypassing change is re-driven through the full workflow (spec, gate,
   reviews) within 72 hours or reverted.
