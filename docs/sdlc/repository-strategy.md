...
```text
fitforge-api     # Backend repository
fitforge-web    # Frontend repository (Next.js app + BFF route handlers)
```

FitForge keeps governance in a third repository, which is the one you are reading:

```text
fitforge         # Governance repository — the constitution, docs/, specs/
```
...
- ASP.NET Core Web API (.NET 10), EF Core, SQL Server migrations, the domain model
- The only database connection in the product
- Backend tests
- Database migrations
...
- The BFF: `app/api/**` route handlers holding the session and aggregating calls to
  `fitforge-api` — never a database connection, never a domain rule
- Frontend tests
...
FitForge uses it because two developers own vertical slices across both tiers: one canonical
place for the law is what keeps their reviews of each other comparable.

## Nested Layout (what FitForge uses)
...
```text
fitforge/                      # governance repo
├── CLAUDE.md  .specify/  docs/  specs/  scripts/
├── fitforge-api/          # independent code repo (ignored by parent)
└── fitforge-web/              # independent code repo (ignored by parent)
```
...
1. Create matching branches (same `NNN-<name>`) in each affected repository — plus the spec
   branch in `fitforge`, the governance repository.
