...
- [ ] T007 Add `builder.Services.AddProblemDetails()` and a global `IExceptionHandler` in `src/FitForge.Api/Infrastructure/` mapping `NotFoundException` → 404, `DomainRuleViolationException` → 422, everything else → 500. Outside Development the document carries no exception message and no stack trace.
